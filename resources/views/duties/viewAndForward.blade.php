...
@push('js')
    ...
    <script>
        ...
        const forwardUrl = "{{ route('duties.view-and-forward.forward', $proforma->proforma_id) }}";
        const verifyUrl = "{{ route('duties.view-and-forward.verify', $proforma->proforma_id) }}";
        const confirmRedirectUrl =
            "{{ route('duties.view-and-forward.index', $tasks['current']['tasks_id']) }}?view=pending";
    </script>
