                        ...
                        <div class="row">
                            ...
                            <div class="col-12 textleft">
                                <b>EIN: {{ $data['ein'] }} &nbsp; Deceased Name : {{ $data['deceased_emp_name'] }}
                                    &nbsp; D.O.E: {{date('d-m-Y', strtotime( $data['deceased_doe'])) }}
                    ...
                    <div class="container">
                        <div class="card-body">
                            ...
                            <div id="form_body" class="displayshow">
                                <form id="descriptiveRoleForm" method="POST" action="" autocomplete="off" enctype="multipart/form-data" class="">
...
@endif
                                    ...
                                    <div class="row textright">
                                        <div class="col-sm-12">
                                            ...
                                            @csrf
                                            @method('POST')
                                            <button disabled id="discard_changes" type="button"
                                                class="btn btn-danger  btn-sm">Discard Changes</button>

                                            <button id="desc_role_next_btn" type="submit"
                                                class="btn btn-primary  btn-sm">Save As Draft</button>
                                            <a href="{{ route('view-family-details-dihas') }}" class=" 
                                            btn btn-success btn-sm" tabindex="-1" role="button" aria-disabled="true"
                                                id="close"><span class="style200">Next</span></a>
    ...
    <!-- The below code is for Discard Changes Button -->
    <div class="modal fade" id="discardChangesModal" tabindex="-1" role="dialog"
        aria-labelledby="discardChangesModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="discardChangesModalLabel">Confirmation</h5>
                </div>
                <div class="modal-body">
                    Are you sure you want to discard the changes in the data?
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-primary" id="confirmDiscardChangesBtn">Ok</button>
                    <button type="button" class="btn btn-secondary" id="confirmCancelfordiscardChanges"
    ...
    document.getElementById('applicant_edu_id').addEventListener('change', function() {
        ...
        if (selectedOption === 'Others') {
            otherEducationDiv.style.display = 'block';
            // Populate the input field with the old value if  other_qualification is available
          //  otherQualificationInput.value = "{{$data['applicant_edu_id'] == '116' ? $data['other_qualification'] : ''}}";
        } else {
            otherEducationDiv.style.display = 'none';
            // Clear the input field if the selected option is not "Others"
...
<script type="text/javascript" src="{{ asset('assets/js/auth/proformaupdate_delete.js') }}"></script>
