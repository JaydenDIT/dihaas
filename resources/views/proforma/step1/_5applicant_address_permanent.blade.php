...
<div class="row col-sm-6 mb-2 ps-5">
    ...
    <div>
        <input type="text" name="applicant_permanent_locality" id="applicant_permanent_locality" placeholder="Locality"
            value="{{ $proforma->applicant_permanent_locality ?? '' }}"
...
<div class="row col-sm-6 mb-2 ps-5">
    ...
    <div>
        <select name="applicant_permanent_state_id" id="applicant_permanent_state_id" class="form-select state_id_flag"
            ...
            <option value="{{$row->state_id}}" {{ $proforma->applicant_permanent_state_id == $row->state_id ? 'selected' : '' }}>{{$row->state_name}}</option>
            @endforeach
        </select>
        <div class="invalid-feedback" role="alert">
            This field is required.
...
<div class="row col-sm-6 mb-2 ps-5">
    ...
    <div>
        <select name="applicant_permanent_district_id" id="applicant_permanent_district_id"
            ...
            <option value="" selected disabled>Choose...</option>
            @if($action == 'edit')
            @foreach($permanent_districts as $row)
            <option value="{{$row->district_id}}" {{ $proforma->applicant_permanent_district_id == $row->district_id ? 'selected' : '' }}>{{$row->district_name}}</option>
            @endforeach
            @endif
        </select>
        <div class="invalid-feedback" role="alert">
...
<div class="row col-sm-6 mb-2 ps-5">
    ...
    <div>
        <select name="applicant_permanent_subdivision_id" id="applicant_permanent_subdivision_id" class="form-select" required>
            <option value="" selected disabled>Choose...</option>
            @if($action == 'edit')
            @foreach($permanent_subdivisions as $row)
            <option value="{{$row->subdivision_id}}" {{ $proforma->applicant_permanent_subdivision_id == $row->subdivision_id ? 'selected' : '' }}>{{$row->subdivision_name}}</option>
            @endforeach
            @endif
        </select>
        <div class="invalid-feedback" role="alert">
...
<div class="row col-sm-6 mb-2 ps-5">
    ...
    <div>
        <input type="text" name="applicant_permanent_pincode" id="applicant_permanent_pincode" placeholder="Pin Code" class="form-control  is_number"
            value="{{ $proforma->applicant_permanent_pincode ?? '' }}"
