$(document).ready(function () {
        ...
        $(document).on(
            "input",
            "#expire_on_duty, #deceased_doe, #deceased_causeofdeath, #applicant_name, #appl_date, #applicant_dob, #relationship, #applicant_mobile, #applicant_edu_id, #applicant_email_id, #emp_addr_lcality, #emp_addr_district, #emp_addr_subdiv, #emp_state, #emp_pincode, #emp_addr_lcality_ret, #emp_addr_district_ret, #emp_addr_subdiv_ret, #emp_state_ret, #emp_pincode_ret, #applicant_desig_id, #applicant_grade, #second_post_id, #second_grade_id, #third_post_id, #third_grade_id, #dept_id_option, #other_qualification",
    ...
    $("#desc_role_next_btn").click(function (e) {
        ...
        $("#confirmSubmit").one("click", function () {
            url: "/ddo-assist/update_proforma", // Update the route to match your Laravel route
                $.ajax({
                    url: "/ddo-assist/update_proforma", // Update the route to match your Laravel route
                    type: "POST",
                    data: formData,
    ...
    $("#deleteBtn").click(function (e) {
        ...
        // Handle OK button click event in the modal
        $("#confirmDiscardBtn").one("click", function () {
            url: "/delete/" + ein, // Update the route to match your Laravel route
                $.ajax({
                    url: "/delete/" + ein, // Update the route to match your Laravel route
                    type: "DELETE",
                    data: formData,
                    success: function (response) {
                        ...
                        window.location.href =
                            "/ddo-assist/enterProformaDetails";
                    },
                    error: function (error) {
                        // Handle error here, if needed
                        alert(
        ...
        // Handle Cancel button click event in the modal
        $("#confirmCancelfordiscard").one("click", function () {
            // Close the modal without fade animation
            $("#discardModal").modal("hide");
        });
    });
...
$("#discard_changes").click(function (e) {
    ...
    // Handle OK button click event in the modal
    $("#confirmDiscardChangesBtn").one("click", function () {
        // Update the route to match your Laravel route
        $.ajax({
            url: "/ddo-assist/discard_Changes", // Update the route to match your Laravel route
            type: "POST",
            data: formData,
            success: function (response) {
                alert(response.message);
    ...
    // Handle Cancel button click event in the modal
    $("#confirmCancelfordiscardChanges").one("click", function () {
        // Close the modal without fade animation
        $("#discardChangesModal").modal("hide");
        document.getElementById("discard_changes").disabled = false;
    });
